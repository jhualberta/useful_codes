...
void analyMPearlyHit()
{
   const char* filename = "FitPartialMP_vertexInWaterFitWater_leadingEdge_1p42_fillBot.root";
   ...
   const char *fitName = "multipathPartial";
   ...
   for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
   {
    ...
    for(Int_t iev=0;iev<nevC; iev++){
     ...
     if(rev.FitResultExists(fitName)) {//find scintFitter exists
        RAT::DS::FitVertex fitVertex = rev.GetFitResult(fitName).GetVertex(0);
        if(rev.GetFitResult(fitName).GetValid())//Global Validity of the fitter !!! NOTE: this is different
        {
 	  ...
 	  { 
	    double posX=(pos_fit.X());double posY=(pos_fit.Y());double posZ=(pos_fit.Z());
            for(unsigned int ipmt=0;ipmt<calpmts.GetCount();ipmt++)
            {
             ...
             double distInUpperTarget_fit = lightPath.GetDistInUpperTarget();
             double distInLowerTarget_fit = lightPath.GetDistInLowerTarget();
             RAT::DU::EffectiveVelocity effectiveVelocity_fit = RAT::DU::Utility::Get()->GetEffectiveVelocity();
             effectiveVelocity_fit.BeginOfRun();
             const double transitTimeFit = groupVelocity.CalcByDistance(distInUpperTarget_fit, distInAV_fit, distInWater_fit+distInLowerTarget_fit);
         ...
         for(vector<int>::iterator it = selectId.begin(); it!= selectId.end(); it++)
         {
           ...
           TVector3 pmtpos1 = pmtInfo.GetPosition(calpmts.GetPMT(*it).GetID());
           ...
           lightPath.CalcByPositionPartial( pos_fit, pmtpos1 );
           double distInInnerAV1 = lightPath.GetDistInInnerAV();
           double distInAV1 = lightPath.GetDistInAV();
           double distInWater1 = lightPath.GetDistInWater();
           double distInUpperTarget1 = lightPath.GetDistInUpperTarget();
           double distInLowerTarget1 = lightPath.GetDistInLowerTarget();
           RAT::DU::EffectiveVelocity effectiveVelocity1 = RAT::DU::Utility::Get()->GetEffectiveVelocity();
           effectiveVelocity1.BeginOfRun();
           const double transitTime1 = groupVelocity.CalcByDistance(distInUpperTarget1, distInAV1, distInWater1+distInLowerTarget1);
           double tres_early = hitTime - transitTime1 - tfit; 
  ...
  TFile *fnew = new TFile("PMT_tres_"+newfile,"recreate");
  fnew->cd();
  //hPMTdist->Write();
  htRes->Write();htRes_early->Write();
