...
//  RAT::DU::LightPathCalculator lightPathMC = RAT::DU::Utility::Get()->GetLightPathCalculator(); // To calculate the light's path
 ...
 //!!! RSP correctiion
  ...
  std::vector<double> *gCosMC = new std::vector<double>;
...
//             // MC
	     ...
	     nhits= ev.GetNhitsCleaned();
			 ...
			 double cosThetaMC =  (pmtpos - pos_mc).Unit()*u_mc;
			 gCosMC->push_back(cosThetaMC);
                         double cosThetaTrue = (pmtpos - pos_mc).Unit()*-1*directionSun;
