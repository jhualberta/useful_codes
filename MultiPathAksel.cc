...
void MultiPath::DumpLikelihood(int index, int gtid, std::string dumpVariable) {
  // make histograms
  TH1F LLHFitPosition("LLHFitPosition","LL Versus PMTID at FitPosition;PMTID", 10000, 0.0, 10000.0);
  TH1F LLHoriginalPosition("LLHoriginalPosition","LL Versus PMTID at originalPosition;PMTID", 10000, 0.0, 10000.0);
  TH1F TresFitPosition("TresFitPosition","Tres vs PMTID for FitPosition;PMTID", 10000, 0.0, 10000.0);
  TH1F TresOriginalPosition("TresOriginalPosition","Tres for OriginalPosition;PMTID", 10000, 0.0, 10000.0);
  TH2F pmtGEO("pmtGEO","PMT Geometry", 628, -M_PI, M_PI, 1000, -1.0, 1.0);

  std::vector<TH1F*> vHists;  //histograms for LL, Derivative and Numerical Derivative
  std::vector<TH2F*> Hist2D; //histograms for LL, Derivative and Numerical Derivative versus PMTID
  ...
  for (size_t ip = 0; ip < nprefix; ip++) {
    for (size_t in = 0; in < npars; in++) {
      ...
      s2 << prefix[ip] << specs[in].GetName().c_str() << "_2_" << index;
      const char *sn2=s2.str().c_str();
      TH2F *h2= new TH2F(sn2,sn2,specs[in].GetNBins(),
              specs[in].GetLowValue(), specs[in].GetHighValue(),10000,0,10000);
      h->SetDirectory(0);
      h2->SetDirectory(0);
      vHists.push_back(h);
      Hist2D.push_back(h2);
  ...
  for (size_t j = 0; j < npars; j++) {
    ...
    for (int k = 0; k < NIncrement; k++) {
      ...
      for (size_t ipmt = 0; ipmt < fPMTReduced.size(); ipmt++) {
        std::vector<double> dbeta(npars);
        double Lpmt = fFunction->Calculate(fPMTReduced[ipmt], trial, dbeta);
        double tres_pmt= fFunction->GetResidual(); // GetResidual
        int PMTid = fSelectedPMTData[ipmt].GetID();
        Hist2D[j]->Fill(trial[j],PMTid,Lpmt);
        Hist2D[j+npars]->Fill(trial[j],PMTid,dbeta[j]);
        Hist2D[j+2*npars]->Fill(trial[j],PMTid,tres_pmt);
      }
    }
