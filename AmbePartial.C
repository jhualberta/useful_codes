...
TVector3 srcPos(1181, -1194, 6713-108.0);
const Float_t C_WATER = 18.34;//cm/ns, for scintillator, 21.6628 ; //speed of light in water
...
string fitName = "partialFitter" ;
Int_t N12TH=4 ;
TTree *theOTree; 

double scaleNhit = 42.857;//300./7;
...
void AmbePartial(const char* inputfile)
{
//  if ( argc != 3 ) {
...
//D Delay
  ...
  // TString Infname = "FitPartial_SNOP_0000252665_000.root";
  TString OUTfname;
//  Infname = argv[2];
//  OUTfname = argv[1];
  RAT::DU::DSReader dsreader(inputfile);
  // To get the run info
  const RAT::DS::Run& run = dsreader.GetRun();
   //Get pmtInfo
   const RAT::DU::PMTInfo& pmtInfo = RAT::DU::Utility::Get()->GetPMTInfo();
   TString Infname(inputfile);
   ...
   theOTree->Branch("srcToPos",    res.srcToPos,    "srcToPos[np]/F");
   ...
   theOTree->Branch("itr",    res.itr,    "itr[np]/F");

   theOTree->Branch("gcosTheta", &res.gcosTheta);
   theOTree->Branch("gtRes", &res.gtRes);
   theOTree->Branch("gPMTX", &res.gPMTX);
   theOTree->Branch("gPMTY", &res.gPMTY);
   theOTree->Branch("gPMTZ", &res.gPMTZ);
   theOTree->Branch("gQHS", &res.gQHS);
   theOTree->Branch("gQHL", &res.gQHL);
  ...
  for(size_t i=0; i<dsreader.GetEntryCount();i++){
    if(i%10000==0) {
      //cout << "Dealing with " << i << "th event" << endl;
    }	    
    ...
    */
    ...
    for(int iev=0;iev<nevC; iev++){
      ...
      if(fitName == "partialFitter"){
           if( nHits>14*scaleNhit && nHits<50*scaleNhit ) foundPrimary = true; //define a primary event
	   else continue;
           ...
           if( nHits>4*scaleNhit && nHits<50*scaleNhit ) foundPrimary = true; //define a primary event
           else continue; 
	   if( !rev.FitResultExists(fitName)) continue;
      ...
      double fom = rev.GetFitResult(fitName).GetFOM("Positionmultipath_scintwater");
      double itrval = rev.GetClassifierResult( "ITR:partialFitter" ).GetClassification( "ITR" );
      double selectedNhits = rev.GetFitResult("partialFitter").GetFOM("Positionmultipath_SelectedNHit_scintwater");
      double nhits = rev.GetNhits();
      double fitenergy = 0;//fVertex.GetEnergy();
	  ...
	  for(size_t kd=i; kd<dsreader.GetEntryCount();kd++){ //search from this event
            if(Beyondtime) break;
	    const RAT::DS::Entry& rdelay = dsreader.GetEntry(kd);
	    for(size_t ievd=0;ievd<rdelay.GetEVCount(); ievd++){
		  ...
		  if (foundDelay) { //do neutron search if within 1000 us
		       ...
		       int DNhits = nHits;
                       ...
                       if(nHits<4*scaleNhit || nHits>25*scaleNhit) continue; //if 25, strange events at high end
                       if(fitName == "partialFitter"){
		       ...
		       //test for -100, 520. This will incease the bkg level but no increase for signal.
	                  ...
	                  res.deltaT[npk]  = deltaT ;
                          ...
                          res.srcToPos[npk] = sqrt((pos_mc.X()-srcPos.X())*(pos_mc.X()-srcPos.X())+(pos_mc.Y()-srcPos.Y())*(pos_mc.Y()-srcPos.Y())+(pos_mc.Z()-108-srcPos.X())*(pos_mc.Z()-108-srcPos.X()));
			  res.logL[npk]    = fom;
...
void NeutronSearch (const RAT::DS::EV& fev, const RAT::DU::PMTInfo& pmtInfo, Float_t tstart, Float_t tend, TVector3 pos_mc) {
       //cout << "Searching neutron now ************"<< endl;
        RAT::DU::LightPathCalculator lightPath = RAT::DU::Utility::Get()->GetLightPathCalculator(); // To calculate the light's path
       //const RAT::DU::GroupVelocity& groupVelocity = RAT::DU::Utility::Get()->GetGroupVelocity(); // To get the group velocity
        const RAT::DU::EffectiveVelocity &effectiveVelocity = RAT::DU::Utility::Get()->GetEffectiveVelocity();
	...
	Float_t tiskz[MAXHITS], tiskz2[MAXHITS], tRes[MAXHITS];
	Float_t qiskz[MAXHITS], qiskz2[MAXHITS];
	TVector3 pos[MAXHITS];
        TVector3 pmtpos[MAXHITS];
	Int_t   index[MAXHITS];
	Float_t hitv_x[MAXHITS]; 
	Float_t hitv_y[MAXHITS];
	Float_t hitv_z[MAXHITS];
	Int_t nhits = 0;
	for (size_t i=0; i<calpmts.GetCount(); i++) {
	    cabiz2[nhits]= calpmts.GetPMT(i).GetID();
            double hitTime = calpmts.GetPMT(i).GetTime();
	    tiskz2[nhits] = hitTime;
	    pmtpos[nhits] = pmtInfo.GetPosition(cabiz2[nhits]);
	    // if(i==1) cout << "PMT position is " << pmtpos[i].X() << endl; 
	    qiskz2[nhits] = calpmts.GetPMT(i).GetQHS();
            res.gPMTX->push_back(pmtpos[nhits].X());
            // if(i==1) cout << "PMT vector position is " << &(res.gPMTX[nhits]) << endl;
	    res.gPMTY->push_back(pmtpos[nhits].Y());
	    res.gPMTZ->push_back(pmtpos[nhits].Z());
            res.gQHS->push_back(calpmts.GetPMT(i).GetQHS());
            res.gQHL->push_back(calpmts.GetPMT(i).GetQHL());
	    double cosTheta = (pos_fit-srcPos).Unit()*(pmtpos[nhits]-pos_fit).Unit(); 
	    res.gcosTheta->push_back(cosTheta);
            nhits++;
	} // Yang uses two loops, no needs
	 
	// TOF and time residual
	for (Int_t i=0; i<nhits; i++) {
          Float_t tof = 0;
          lightPath.CalcByPositionPartial( pos_mc, pmtInfo.GetPosition(calpmts.GetPMT(i).GetID()));//pmtpos[i], pos_mc );
          ...
          tof = transitTime;
          res.gtRes->push_back(calpmts.GetPMT(i).GetTime() - transitTime - tfit);
	  // tof = TMath::Sqrt((pmtpos[i].X()-pos_mc.X())*(pmtpos[i].X()-pos_mc.X())+(pmtpos[i].Y()-pos_mc.Y())*(pmtpos[i].Y()-pos_mc.Y())+(pmtpos[i].Z()-pos_mc.Z())*(pmtpos[i].Z()-pos_mc.Z()))/C_WATER/10.; //mm to cm
          tiskz2[i] -= tof;
	}
	...
	Float_t  t0 = -1;
	Int_t    N12i = 0, N12 = 0;
	Int_t    Nc = 0, Nback = 0, Nlow = 0; // Neff, Nc1, NhighQ, NlowQ ;
	Float_t  trms = 0, phirms = 0, thetam = 0, distan = 0;// ratio;
...
Bool_t CheckBackHits (Float_t *ux, Float_t *uy, Float_t *uz, Int_t *flag,
		                                            Int_t N12, Int_t &ncut, Float_t angle)
{
    const Float_t THR = TMath::Cos(angle*TMath::Pi()/180.);
...
Double_t GetTriggeredEventTime(const RAT::DS::EV& tev) {
  /// for partial-phase, the GPS is broken!! use other trigger time
	
  long int time = tev.GetClockCount50();
  return time*20;//
