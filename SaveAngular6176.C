...
void SaveAngular6176(TString infile_name, double xsrc, double ysrc, double zsrc)
{
  ...
  //int choice = 2;
  //if(fabs(srcPos.Mag()-6000)<1000)
  //{
  // choice = 1;
  //cout<<"1 m cut!! "<<endl;
  //}	  
  double posx, posy, posz, energy, posRad, itr, dirx, diry, dirz, beta14, scaleLogL;
  UInt_t nhits;
  ...
  for(int i =0;i<Tdata->GetEntries();i++)
  {
    ...
    /// if(nhits>5 && itr>0.55 && beta14<0.95 && beta14>-0.12 && energy>3.5)
    {
      ...
      if(diff>1000 && diff<2300) hAngularCut0->Fill(Xdiff.Unit()*Dir);
