...
fit9 = FTP
*/
double PMTsep = 1000;//50 cm
double tResCut[2]={-2.5,5.0};
using namespace std;
// Use 16N data for calculating vg = 2*R_psup/(T1+T2)
void analyCalTimeOfFlight()
{  
   double correctParam[]={1,1,1,0.9838,-63.375};
   double driveP0 = correctParam[3];
   double driveP1 = correctParam[4];
   TString filename = "FitMultiWater1p40_Analysis_r0000100934_s001_p000.root";//Merged_FitMulti_TimeCor_Analysis_r0000100934.root
   TFile *f1 = new TFile(filename);
   RAT::DU::DSReader dsReader("FitMultiWater1p40_Analysis_r0000100934_s001_p000.root");
   TVector3 sourcePos; //!!Note: use the position get from reflection fit
   //sourcePos.SetXYZ(-186,256,-4397.8);
   //sourcePos.SetXYZ(-178.83, 297.96, 15.45);
   sourcePos.SetXYZ(-186, 256, 26);
...
//drive cor with trig cut
   ...
   TH1F* htRes = new TH1F("htRes", "Time Residue", 800,-100,300);
   TH1F* htRes_trig = new TH1F("htRes_trig", "Time Residue, trig", 800,-100,300);
   ...
   TH2F *htResVsPMTposx = new TH2F("htResVsPMTposx","htRes vs PMTposX, trig",1800,-9000,9000,800,-100,300);
   TH2F *htResVsPMTposy = new TH2F("htResVsPMTposy","htRes vs PMTposY",1800,-9000,9000,800,-100,300);
   TH2F *htResVsPMTposz = new TH2F("htResVsPMTposz","htRes vs PMTposZ",1800,-9000,9000,800,-100,300);
   TH2F *htResVsPMTphi = new TH2F("htResVsPMTphi","htRes vs PMTphi",628,-TMath::Pi(),TMath::Pi(),800,-100,300);
   TH2F *htResVsPMTtheta = new TH2F("htResVsPMTtheta","htRes vs PMTtheta",1000,-1,1,800,-100,300);
   
   TH1F *htRes_inPMTsep = new TH1F("htRes_inPMTsep","tRes within 50 cm",800,-100,300);
   TH2F *htRes_inPMTsepVsPMTposx = new TH2F("htRes_inVsPMTposx","tRes within 50 cm vs PMTposX",1800,-9000,9000,800,-100,300);
   TH2F *htRes_inPMTsepVsPMTposy = new TH2F("htRes_inVsPMTposy","tRes within 50 cm vs PMTposY",1800,-9000,9000,800,-100,300);
   TH2F *htRes_inPMTsepVsPMTposz = new TH2F("htRes_inVsPMTposz","tRes within 50 cm vs PMTposZ",1800,-9000,9000,800,-100,300);

   TH1F *htRes_oppPMTsep = new TH1F("htRes_oppPMTsep","tRes out 50 cm",800,-100,300);
   TH2F *htRes_oppPMTsepVsPMTposx = new TH2F("htRes_oppVsPMTposx","tRes opp 50 cm vs PMTposX",1800,-9000,9000,800,-100,300);
   TH2F *htRes_oppPMTsepVsPMTposy = new TH2F("htRes_oppVsPMTposy","tRes opp 50 cm vs PMTposY",1800,-9000,9000,800,-100,300);
   TH2F *htRes_oppPMTsepVsPMTposz = new TH2F("htRes_oppVsPMTposz","tRes opp 50 cm vs PMTposZ",1800,-9000,9000,800,-100,300);

   TH1F *htPrompt_inPMTsep = new TH1F("htPrompt_inPMTsep","",800,-100,300);
   TH1F *htPrompt_oppPMTsep = new TH1F("htPrompt_oppPMTsep","",800,-100,300);
   ...
   TH2F *hDiffTOFvsPMTposx = new TH2F("hDiffTOFvsPMTposx","difference of TOF vs PMTposX",1800,-9000,9000,500,-100,300); 
   ...
   TH2F *htResVsPMTposx_trig = new TH2F("htResVsPMTposx_trig","htRes vs PMTposX, trig",1800,-9000,9000,800,-100,300);
   TH2F *htResVsPMTposy_trig = new TH2F("htResVsPMTposy_trig","htRes vs PMTposY, trig",1800,-9000,9000,800,-100,300);
   TH2F *htResVsPMTposz_trig = new TH2F("htResVsPMTposz_trig","htRes vs PMTposZ, trig",1800,-9000,9000,800,-100,300);
   TH2F *htResVsPMTphi_trig = new TH2F("htResVsPMTphi_trig","htRes vs PMTphi, trig",628,-TMath::Pi(),TMath::Pi(),800,-100,300);
   TH2F *htResVsPMTtheta_trig = new TH2F("htResVsPMTtheta_trig","htRes vs PMTtheta, trig",1000,-1,1,800,-100,300);
     ...
     if( RAT::EventIsClean( rev, dcAnalysisWord ) )
     {
      ...
      if(specFitValid){
       ...
       if(pos_fit.Mag()<9000){  //fit valid: inside cavity, specified by MPW fitter
		...
		pos_cor = driveP0*pos_fit+driveP1*u_fit; //cout<<pos_cor.Unit()*sourcePos.Unit()<<endl;
        ...
        for(unsigned int ipmt=0;ipmt<calpmts.GetFECDCount();ipmt++)//do FECD cuts
        {
	      if(calpmts.GetFECDPMT(ipmt).GetID()==fecdID)
	      {
		   ...
		   for(unsigned int ipmt=0;ipmt<calpmts.GetCount();ipmt++)
		   {
			...
			double tRes = hitTime-fitVertex.GetTime()-(pmtpos-sourcePos).Mag()/grVelocity;
		   ...
		   //pmt cuts
			for(unsigned int jpmt=0;jpmt<calpmts.GetCount();jpmt++)
			{
			  ...
			  double tResj= hitTimej-fitVertex.GetTime()-(pmtposj-sourcePos).Mag()/grVelocity;
			  		 
			  if(pmtposj.Unit()*pmtpos.Unit()>1-PMTsep*PMTsep/(2*rPSUP*rPSUP) && pmtposj.Unit()*pmtpos.Unit()!=1)
			  {
				htRes_inPMTsep->Fill(tResj);
				htRes_inPMTsepVsPMTposx->Fill(pmtposj.X(),tResj);
				htRes_inPMTsepVsPMTposy->Fill(pmtposj.Y(),tResj);
				htRes_inPMTsepVsPMTposz->Fill(pmtposj.Z(),tResj);
				hPMTphiTheta_inCone->Fill(pmtposj.Phi(),cos(pmtposj.Theta()));
				if(tResj<tResCut[1] && tResj>tResCut[0])//get prompt light
				{
				 htPrompt_inPMTsep->Fill((pmtposj-sourcePos).Mag()/grVelocity);//hitTimej-tResj-fitVertex.GetTime());
				 tPrompt1.push_back((pmtposj-sourcePos).Mag()/grVelocity);//hitTimej-tResj-fitVertex.GetTime());
				} 
			  }
			  if(-pmtposj.Unit()*pmtpos.Unit()>1-PMTsep*PMTsep/(2*rPSUP*rPSUP) && pmtposj.Unit()*pmtpos.Unit()!=1)
			  {
				htRes_oppPMTsep->Fill(hitTimej-tResj-fitVertex.GetTime());
				htRes_oppPMTsepVsPMTposx->Fill(pmtposj.X(),tResj);
				htRes_oppPMTsepVsPMTposy->Fill(pmtposj.Y(),tResj);
				htRes_oppPMTsepVsPMTposz->Fill(pmtposj.Z(),tResj);
				hPMTphiTheta_oppCone->Fill(pmtposj.Phi(),cos(pmtposj.Theta()));
				if(tResj<tResCut[1] && tResj>tResCut[0])//get prompt light
				{
				 htPrompt_oppPMTsep->Fill((pmtposj-sourcePos).Mag()/grVelocity);//hitTimej-tResj-fitVertex.GetTime());
				 tPrompt2.push_back((pmtposj-sourcePos).Mag()/grVelocity);//hitTimej-tResj-fitVertex.GetTime());
				 }
			  }
			}//loop PMT j
			...
			if(t1prompt!=0 && t2prompt!=0)
			{
				...
				double tDiff = t1prompt - t2prompt;
				hSumTOFvsPMTposx->Fill(pmtpos.X(),tSum); //calculate average 
				hSumTOFvsPMTposy->Fill(pmtpos.Y(),tSum);    
				hSumTOFvsPMTposz->Fill(pmtpos.Z(),tSum); 
				hDiffTOFvsPMTposx->Fill(pmtpos.X(),tDiff);
 			 	hDiffTOFvsPMTposy->Fill(pmtpos.Y(),tDiff);
				hDiffTOFvsPMTposz->Fill(pmtpos.Z(),tDiff);
		}		
...
//  get->Draw("ds.evs.calPMTs.fecd.GetID()>>hFECD");
 ...
 std::cout<<"fitValid && FECD && trigger/fitValid && FECD: "<<countTimeWindow<<"/"<<countSuccess<<" ratio: "<<countTimeWindow/countSuccess*100<<std::endl;
  ...
  htRes_inPMTsepVsPMTposx->Write();htRes_inPMTsepVsPMTposy->Write();htRes_inPMTsepVsPMTposz->Write();
  htRes_oppPMTsepVsPMTposx->Write();htRes_oppPMTsepVsPMTposy->Write();htRes_oppPMTsepVsPMTposz->Write();
  ...
  hDiffTOFvsPMTposx->Write();hDiffTOFvsPMTposy->Write();hDiffTOFvsPMTposz->Write();
  fp->Close();
