...
nentries = 5000 #data.GetEntries();           
update = int(nentries/10);
fout = TFile("DanielRecalcTimeWin_L6Cuts_"+fileName, "RECREATE")
###########################################################################################################################
...
HsubpeakTime = TH1F("HsubpeakTime","subpeak time, cut level 6", 20000, 0, 20000)
HpulseTime = TH1F("HpulseTime","pulse time, cut level 6", 20000, -4000, 16000)
...
for event in range (nentries):
    ...
    if DS.GetTSCount() > 0 and DS.GetEVCount() > 0 and DS.GetCALCount() > 0:
        ...
        try:CalTrigTime = CAL.GetEventTime() #CalibratedTriggerTime() ##CAL.GetEventTime()
        except:continue
        ...
        for ipmt in range (CAL.GetPMTCount()): ### loop PMTs
             ...
             for ipulse in range (npulse):
                  ...
                  for ipeak in range(nsubpulse): ### loop subpeak
                       ...
                       if pulseTime > startPromptIntegral and pulseTime < endPromptIntegral:
                       ...
                       if pulseTime > start400nsIntegral and pulseTime < end400nsIntegral:
                           total400nsCharge += subQ 

                       if pulseTime > start5000nsIntegral and pulseTime < end5000nsIntegral: 
                           total5000nsCharge += subQ
                           pmt_has_5000ns_pulse = True

                       if pulseTime > endPromptIntegral and pulseTime < endIntegral:
                           pmt_has_late_pulse = True

                       if pulseTime > alphaStartIntegral and pulseTime < alphaEndIntegral:
                           alphaCharge += subQ 
                           if pulseTime < alphaEndPromptIntegral:
