...
if __name__=='__main__':
    fout2 = TFile("savePMTfancyMap.root","recreate")
    ...
    ### event position
    evt_phi = [1.41, 1.36, 1.36, 1.46, 1.42]
    evt_costheta = [-0.69, -0.68, -0.66, -0.55, -0.55]
    nEvent = 5
    array_evt_phi = array('f', evt_phi)
    array_evt_costheta = array('f', evt_costheta)
    gr_evt = TGraph(nEvent, array_evt_phi, array_evt_costheta)
    gr_evt.SetMarkerStyle(3)
    gr_evt.SetMarkerColor(kBlue)
    gr_evt.SetMarkerSize(2)
    gr_evt.Draw("P same")
    #raw_input()
