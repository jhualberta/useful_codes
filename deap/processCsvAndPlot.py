...
excludeOn = True ## turn on the Exclude on Edges
x0 = 4712; y0 = 0; x1 = 4790; y1 = 45; ## for top edge strange noise point
excludeROI = [x0,x1,y0,y1] ##rectangular ROI, not counting the particles inside, to remove some odd regions
...
for fname in files:
    ...
    with open(fname) as csvfile:
        ...
        for row in reader:
            xPos = float(row['X'])
            yPos = float(row['Y'])
            if excludeOn and x0<xPos and xPos<x1 and yPos>0 and yPos<y1:
                  print("noise detected; remove this edge noise in topEdge"+fname[fname.index('Edge')+4:fname.index('Edge')+6])
                  continue ## don't counting this region
            area = float(row['Area'])
            ...
            if mass>15:
                print("???? warning: find large mass particles>15 ng, m="+str(round(mass,2))+" ng. please check this file: topEdge"+fname[fname.index('Edge')+4:fname.index('Edge')+6])
            if area>1000:
                print("???? warning: find large area particles>1000 um^2, S="+str(round(area,2))+" mm^2. please check this file: topEdge"+fname[fname.index('Edge')+4:fname.index('Edge')+6])
        if len(data1)>warning_points:
            print("warning: find "+str( len(data1) )+", more than "+ str(warning_points) +" dusts, please check this file: "+fname)
...
print("mass " + str(round(countMass,1)) + "ng.")
### totalArea is in mm^2, so switch to 1./100 cm^2
print("Density "+str(round(countMass/(totalArea/100*countPic),1))+" ng/cm^2")
