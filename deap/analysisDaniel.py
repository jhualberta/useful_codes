...
for event in range (nentries):
    ...
    if DS.GetTSCount() > 0 and DS.GetEVCount() > 0 and DS.GetCALCount() > 0:
        ...
        flag=0
        nSCBayes = CAL.GetNSCBayes()
        qPE = CAL.GetQPE()
        fprompt = CAL.GetFprompt()
        rprompt60Bayes = EV.GetRprompt60Bayes()
        
        numEarlyPulses = CAL.numEarlyPulses
        subeventN = CAL.GetSubeventCount() 
        eventTime = CAL.GetEventTime()
        # calCuts = CAL.GetCuts().GetCutWord()
        H2_nSCBayes_rprompt60Bayes_0  .Fill(nSCBayes, rprompt60Bayes)
        ...
        if numEarlyPulses <= 3:
              H2_nSCBayes_rprompt60Bayes_4  .Fill(nSCBayes, rprompt60Bayes)
              H2_qpe_fprompt_4              .Fill(qPE     , fprompt       )
              if subeventN==1:
                     H2_nSCBayes_rprompt60Bayes_5  .Fill(nSCBayes, rprompt60Bayes)
                     H2_qpe_fprompt_5              .Fill(qPE     , fprompt       )
                     if eventTime > 2250  and eventTime < 2700:
                          H2_nSCBayes_rprompt60Bayes_6  .Fill(nSCBayes, rprompt60Bayes)
                          H2_qpe_fprompt_6              .Fill(qPE     , fprompt       )
                          if qPE>60:
                                H2_nSCBayes_rprompt60Bayes_7  .Fill(nSCBayes, rprompt60Bayes)
