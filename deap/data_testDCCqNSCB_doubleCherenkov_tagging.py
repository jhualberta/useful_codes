...
fout = TFile("CheckDcc_qnscb_"+fileName,"RECREATE");
...
for event in range(nentries):
        ...
        if DS.GetTSCount() > 0 and DS.GetEVCount() > 0 and DS.GetCALCount() > 0:
            ...
            for ipmt in range(nPMT):
                ...
                #NOTE: use pmt.qNSCB
                pmt_qPE = rpmt.GetQPE()
                pmt_qNSCB = rpmt.GetNSCBayes() # might use pmt.GetPromptQ() as alternative
                ...
                pmtCharge[pmtid] = pmt_qNSCB ### !!! use pmt.qNSCB
            ...
            dbCherenkov_pmtMaxpe = maxQ/nscb # the charge of the brightest pmt/event qPE
