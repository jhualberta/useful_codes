...
const double trueLevel = 4700;
void analyPartialSplitZ()
{  
   const char* filename = "Fit5100mm_simu_5MeVbeta_0p5PPO_IsoTop_splitZ4700_1e4evt.root";
...
//check pos.Mag() vs fitTime
  ...
  //for N16 source 
   ...
   TH2F* hfitYZ = new TH2F("hfitYZ", "SNO+  fitted X vs Z, FECD cut", 2000,-9000,9000,2000,-9000,9000);
   ...
   TH2F* hmcXY = new TH2F("hmcXY", "MC x vs y", 2000,-9000,9000, 2000,-9000,18000);
   TH2F* hmcXZ = new TH2F("hmcXZ", "MC x vs z", 2000,-9000,9000, 2000,-9000,18000);
   TH2F* hmcYZ = new TH2F("hmcYZ", "MC y vs z", 2000,-9000,9000, 2000,-9000,18000);
   TH2F* hmcRZ = new TH2F("hmcRZ", "MC r vs z", 1000, 0,9000, 2000,-9000,18000);
   TH2F* hmcRhoZ = new TH2F("hmcRhoZ", "MC rho vs Z", 1000, 0,9000,2000,-9000,18000);
   ...
   // odd events between true water level and splitZ
   TH1F* hDeltaX = new TH1F("hDeltaX", "SNO+ fitted - mc X, FECD+trig cut", 2000,-9000,9000);
   TH1F* hDeltaY = new TH1F("hDeltaY", "SNO+ fitted - mc Y, FECD+trig cut", 2000,-9000,9000);
   TH1F* hDeltaZ = new TH1F("hDeltaZ", "SNO+ fitted - mc Z, FECD+trig cut", 2000,-9000,9000);
   TH1F* hDeltaR = new TH1F("hDeltaR", "SNO+ (Xfit-Xmc).Mag(), FECD+trig cut", 1000,0,9000);
   TH2F* hRhoZodd = new TH2F("hRhoZodd", "SNO+ odd Rho vs Z, FECD cut", 1000, 0,9000,2000,-9000,9000);
   TH2F* hMCRhoZodd = new TH2F("hMCRhoZodd", "SNO+ odd Rho vs Z, FECD cut", 1000, 0,9000,2000,-9000,9000);
   TH1F* hNhitodd = new TH1F("hNhitodd", "nhit odd", 1000, 0, 1000);
   TH1F* hscaleLogLodd= new TH1F("hscaleLogLodd", "scaled logL odd", 200, 0, 20);
   ...
   int countMC = 0;
   int countFit = 0;
   int countFitWaterLevel = 0;
   int countFitAboveWater = 0;
   int countFitBelowWater = 0;
   int countFitOutAV = 0;

   vector<unsigned int> checkdata;
   // first loop to pick up mc
   for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
   {
	...
	TVector3 pos_mc = rmcparticle.GetPosition();
        //!!! correction MC Z
        pos_mc.SetZ(pos_mc.Z()-108);
          ...
          try{
            ...
            if(fitVertex.ValidPosition())// && fVertex1.ValidDirection())//NOTE: check fitPosition Valid !!
            {
              for(unsigned int ipmt=0;ipmt<calpmts.GetFECDCount();ipmt++)//do FECD cuts
              {
                 ...
                 {
                   if(pos_mc.Mag()<6005 && pos_mc.Z()>trueLevel && pos_mc.Z()<5100)
                   //if(pos_mc.Mag()<6005 && pos_mc.Z()<trueLevel && pos_mc.Z()>5100)
      	           {
                     countMC++;
                     hMCRhoZodd->Fill( pos_mc.Perp(), pos_mc.Z() );
                     checkdata.push_back(gtid);
  		    }
                 }
	      }
	    }	
   ...
   std::vector<unsigned int>::iterator it11;
   for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
   {
    ...
    for(Int_t iev=0;iev<nevC; iev++){
     ...
     //if( RAT::EventIsClean( rev, dcAnalysisWord ) )
     {
	  ...
	  TVector3 posfit_cor;
	  posfit_cor.SetXYZ(pos_fit.X(),pos_fit.Y(),pos_fit.Z()-108); 
         ...
         // } // if valid direction
	      ...
	      if(calpmts.GetFECDPMT(ipmt).GetID()==fecdID)
	      {
		   ...
		   //  hMCRhoZodd->Fill( pos_mc.Perp(), pos_mc.Z() );
                   //  cout<<gtid<<",";
                   //}
                   it11 = find(checkdata.begin(), checkdata.end(), gtid);
                   //hDeltaX->Fill(posX-pos_mc.X());
                   //hDeltaY->Fill(posY-pos_mc.Y());
                   //hDeltaZ->Fill(posfit_cor.Z()-pos_mc.Z());
		   if (it11 != checkdata.end())
                   {
		     hNhitodd->Fill(nhits);
		     hscaleLogLodd->Fill(scaleLogL);
		     hDeltaX->Fill(posX-pos_mc.X());
		     hDeltaY->Fill(posY-pos_mc.Y());
		     hDeltaZ->Fill(posfit_cor.Z()-pos_mc.Z());
		     hDeltaR->Fill( (posfit_cor-pos_mc).X());
                     hRhoZodd->Fill( posfit_cor.Perp(), posfit_cor.Z() );
		     //if(posfit_cor.Mag()<6005 && posfit_cor.Z()<trueLevel && posfit_cor.Z()>5100)
		     if(posfit_cor.Mag()<6005 && posfit_cor.Z()>trueLevel && posfit_cor.Z()<5100)
                     {
                       countFit++;
		       // hRhoZodd->Fill( posfit_cor.Perp(), posfit_cor.Z() );
                     ...
                     if(posfit_cor.Mag()<6005 && posfit_cor.Z()>5100-10 && posfit_cor.Z()<5100+10) countFitWaterLevel++;
                     if(posfit_cor.Mag()<6005 && posfit_cor.Z()>5100+10) countFitAboveWater++;
                     if(posfit_cor.Mag()<6005 && posfit_cor.Z()<trueLevel) countFitBelowWater++;
		     if(posfit_cor.Mag()>6005) countFitOutAV++; 
		   }
		   ...
		   hmcXY->Fill(pos_mc.X(),pos_mc.Y());
                   hmcXZ->Fill(pos_mc.X(),pos_mc.Z());
                   hmcYZ->Fill(pos_mc.Y(),pos_mc.Z());
                   hmcRZ->Fill(pos_mc.Mag(),pos_mc.Z());
                   hmcRhoZ->Fill(pos_mc.Perp(),pos_mc.Z());
...
//  std::cout<<"fitValid && FECD/FECD total(9188+9027): "<<countSuccess<<"/"<<hFECD->GetEntries()<<" ratio: "<<countSuccess/hFECD->GetEntries()*100<<std::endl; //fitValid && FECD/ FECD, FECD means FECD == 9188, FECD total means 9188+9207
  ...
  std::cout<<"mc "<<countMC<<" fit "<<countFit<<"("<<double(countFit)/countMC*100<<"%) around(5100+-10) "<<countFitWaterLevel<<" ("<<double(countFitWaterLevel)/countMC*100<<"%)"<<std::endl;
  std::cout<<"above(>5100+10) "<<countFitAboveWater<<"("<<double(countFitAboveWater)/countMC*100<<"%)"<<std::endl;
  std::cout<<"below(<trueLevel) "<<countFitBelowWater<<"("<<double(countFitBelowWater)/countMC*100<<"%) outside AV "<<countFitOutAV<<" ("<<double(countFitOutAV)/countMC*100<<"%)"<<std::endl;
  TString newfilename = "ResolPartialMC_"+TString(filename); 
  ...
  hTrig->Write(); 
  hfitX->Write();hfitY->Write();hfitZ->Write();hfitXY->Write();hfitXZ->Write();hfitYZ->Write();hfitRhoZ->Write();hfitRZ->Write();hPosMag->Write();
  hfitX_trig->Write();hfitY_trig->Write();hfitZ_trig->Write();hfitXZ_trig->Write();hfitRZ_trig->Write();hPosMag_trig->Write();

  hDeltaX->Write(); hDeltaX->GetXaxis()->SetRangeUser(-2000,2000);
  hDeltaY->Write(); hDeltaY->GetXaxis()->SetRangeUser(-2000,2000);
  hDeltaZ->Write(); hDeltaZ->GetXaxis()->SetRangeUser(-2000,2000);
  hDeltaR->Write();
  hNhitodd->Write();
  hscaleLogLodd->Write();

  hRhoZodd->GetYaxis()->SetRangeUser(3000,6000);hRhoZodd->GetXaxis()->SetRangeUser(0,6000);
  hRhoZodd->Write();
  hMCRhoZodd->GetYaxis()->SetRangeUser(3000,6000);hMCRhoZodd->GetXaxis()->SetRangeUser(0,6000);
  hMCRhoZodd->Write();
  ...
  hmcXY->Write(); hmcXZ->Write(); hmcYZ->Write(); hmcRZ->Write(); hmcRhoZ->Write();

  hNhitsVsX->Write();hNhitsVsY->Write();hNhitsVsZ->Write();hNhitsVsR->Write();

  hNhitsFECD->Write();hPosXvsTime->Write();
  hFOM->Write();hFitTime->Write();hHitTime->Write();
  ...
  htRes_1p40->Write();htRes_1p378->Write();
