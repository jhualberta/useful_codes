	...
	if (iEVCount) {
	     ...
	     RAT::DS::DataQCFlags dcflags = ev.GetDataCleaningFlags();
             ...
             {	  
               ...
               {
                 fecdID = calpmts.GetFECDPMT(ipmt).GetID();vfecd->push_back(fecdID);
               }
               if(((TriggerType&2)==2))// || ((TriggerType&18)==18))
               {
                 if(ev.FitResultExists("multipath")) // fit exists 
                 {  // It needs to exist 
                  try {
