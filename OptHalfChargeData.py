...
fout = TFile("OptNoQcuts_"+file0,"RECREATE");
...
for Files in AllFiles:
	...
	# Low Level cuts
			...
			H2_CosTheta_Phi.Reset()
                        ...
                        # if (qTopRing + qSecondRing)/qPE > 0.04: continue;
                        # if (qBotRing + qSecondBotRing + qThirdBotRing)/qPE > 0.1: continue;
			...
			pmtTime = list(pmtTime)
                       ### !!! save PMT info into lists, index is not sorted by PMTid
                       check_charges = []
                       check_pmtTime = []
                       check_pmtPhi = []; check_pmtCosTheta = []
                       ### fill this event
			for i in range(255):
                             if charges[i] > 0: ### Fill all the PMTs in this event
                                  check_charges.append(charges[i]) 
                                  check_pmtTime.append(pmtTime[i])
                                  check_pmtPhi.append(savePmtPhi[i]); check_pmtCosTheta.append(savePmtCosTheta[i]); 
			...
			Time_CosTheta_Phi3.Reset()
                        ...
                        select_pmtList = q_sort[0:6] ## look into the 6 largest q PMTs
                        qmax = select_pmtList[0] ## the 1st is the largest
                        if qmax<100:
                            print "no obvious hit points, pass this event !!!"
                            count_ncluster = 0
                            continue

                        ind_max = []
                        for i in range(len(select_pmtList)):
                             ind = check_charges.index(select_pmtList[i])
                             ind_max.append(ind)
                        ### in0 is the largest, in1 is the 2nd largest, ... (reversed)     
                        list_index = []
                        for i in range(6):
                            list_index.append(ind_max[i])

                        select_pmtPos = []
                        for i in range(6):
                             phi = check_pmtPhi[list_index[i]] 
                             cost = check_pmtCosTheta[list_index[i]]
                             th = arccos(cost)
                        ...
                        #print "======= checking event", evtID[0], "qmax", select_pmtList 
                        ## check dist, angles between 0,1; 0,2; ...
                        for i in range(1,6):
                            if(select_pmtList[i]>100):
                                dist = (select_pmtPos[i] - pmt0_pos).Mag()
                                pmt_dist.append(dist) ## note: the index-0 element saves the (q0,q1);
                                angle = arccos(pmt0_pos.Unit()*select_pmtPos[i].Unit())
                                pmt_angles.append(angle)
                            else: 
                                #print "only up to PMTs", i
                                break; # the rest should be lower charges, abandon
                        ...
                        cluster_phi = []
                        cluster_cosTheta = []
                        searchPMTid_cluster1 = [] ### search PMT id by looking up in charges[]
                        searchPMTid_cluster2 = []

                        searchPMTid_cluster1.append(charges.index(select_pmtList[0])) # save the qmax first; this is the seed of the 1st cluster

                        seed_cluster2 = -10000 # if there is a PMT far away from qmax or q0 PMT, is this PMT the seed of the 2nd cluster?

                        #print "pmtList", select_pmtList
                        for i in range(len(pmt_dist)):
                                angle = pmt_angles[i] # qmax pmt (pmt0) to the pmt1, from 1 to 5;
                                dist = pmt_dist[i]
                                if angle<(16.3*pi/180) and dist<250:
                                    #print "eventID", evtID[0],"possible single cluster"
                                    count_nearPMT += 1
                                    searchPMTid_cluster1.append(charges.index(select_pmtList[i+1])) # select_pmtList from 1, 2, ... compared to 0
                                    continue
                                if angle>(16.3*pi/180) and dist>250:
                                    #print "eventID", evtID[0],"double cluster! dist = ", dist
                                    flag_double = 1
                                    searchPMTid_cluster2.append(charges.index(select_pmtList[i+1])) # select_pmtList from 1, 2, ... compared to 0
                                    seed_cluster2 = i+1
                                    print "seed2", seed_cluster2
                                    break;

                        ## check the 2nd cluster from the seed
                        if flag_double == 1:
                                pos_seed_cluster2 = select_pmtPos[seed_cluster2]
                                for i in range(seed_cluster2,len(select_pmtList)):# automatically ensures seed_cluster2<=len(select_pmtList)
                                      if i+1<len(pmt_dist):
                                           angle = arccos(pos_seed_cluster2.Unit()*select_pmtPos[i+1].Unit())
                                           dist = (select_pmtPos[i+1] - pos_seed_cluster2).Mag()
                                           if angle<(16.3*pi/180) and dist<250:
                                                searchPMTid_cluster2.append(charges.index(select_pmtList[i+1]))
                        ...
                        if flag_double == 1:
                             count_ncluster = 2 ## count for single cluster
                             ncluster[0] = count_ncluster
                             id1 = charges.index(qmax)  #select_pmtList[1])
