...
const double driveCor_p0 = 0.995765; // drive parameters in MPW; see Water-Phase-Unidoc; x_correct = x_fit*p0+u_fit*p1.   ,
const double driveCor_p1 = -63.826;//-38.8275;
...
//  }
  // Read in file
  string Infname = "FitMPW_109133_s000_p000.root";
  TString OUTfname;
//  Infname = argv[2];
//  OUTfname = argv[1];
  ...
  const RAT::DS::Run& run = dsreader.GetRun();
   //Get pmtInfo
   const RAT::DU::PMTInfo& pmtInfo = RAT::DU::Utility::Get()->GetPMTInfo();
   TString savefile = "SaveMpwAmBe_"+TString(Infname);
  ...
  for(size_t i=0; i<dsreader.GetEntryCount();i++) {
    ...
    */
    ...
    for(int iev=0;iev<nevC; iev++){
      ...
      if(fitName == "multipath_waterposition"){
           if(nHits>14 && nHits<50.) foundPrimary = true; //define a primary event
	   else continue;
           ...
           if( !rev.FitResultExists("multipathdirection")) continue;
           if( !rev.GetFitResult("multipathdirection").GetVertex(0).ContainsDirection() ) continue;
           if( !rev.GetFitResult("multipathdirection").GetVertex(0).ValidDirection() ) continue;
      }
      else{
      ...
      pos_fit = fVertex.GetPosition(); //vertex of primary event, reconstructed case
      double fom = fResult.GetFOM("multipath_waterposition");
      double selectedNhits = fResult.GetFOM("multipath_SelectedNHit_waterposition");
      double fomScale = fom/selectedNhits;
      double nhits = rev.GetNhitsCleaned();
      try{
      RAT::DS::FitVertex fVertex1 = rev.GetFitResult("multipathdirection").GetVertex(0);
      u_fit = fVertex1.GetDirection();
      pos_fit = driveCor_p0*pos_fit+driveCor_p1*u_fit;
	  ...
	  for(int kd=i; kd<dsreader.GetEntryCount();kd++){ //search from this event
	    ...
	    for(int ievd=0;ievd<rdelay.GetEVCount(); ievd++){
		  ...
		  if (foundDelay) { //do neutron search if within 1000 us
		       ...
		       nHits = revdelay.GetNhitsCleaned();
		       int DNhits = nHits;
                       ...
                       res.Clear();
                       //search
                       NeutronSearch(revdelay, pmtInfo, 0., 420., pos_fit);
		       //test for -100, 520. This will incease the bkg level but no increase for signal.
	                  ...
	                  res.deltaT[npk]  = deltaT ;
                          res.posX[npk]    = pos_fit.X();
                          res.posY[npk]    = pos_fit.Y();
                          res.posZ[npk]    = pos_fit.Z();
                          res.energy[npk]  = fitenergy;
                          res.logL[npk] = fom;
                          res.selectedNhits[npk] = selectedNhits;
                          res.logLscale[npk] = fomScale;
                       }
                       theOTree->Fill();
                       ///////////////////fill finish
		  }
