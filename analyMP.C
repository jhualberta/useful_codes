...
void analyMP()
{  
   //const char* filename = "FitAP1p385_MCrat652_partialWater_5MeVbeta_x0_y0_z-3000_level-2408.root";
   //const char* filename = "FitAP1p385_MC_partialscint_5MeVbeta_x0_y0_z-3000_wl-2408.root";
   const char* filename = "FitMP_MC_partialscint_2p5MeVbeta_x0_y0_z-3000_level0.root";
 	...
 	{ countFitValid++;//count events of fitValid 
	  double posX=(pos_fit.X());double posY=(pos_fit.Y());double posZ=(pos_fit.Z());
 	  hFitTime_noFECD->Fill(tfit);
          if(posZ>-1 && posZ<1 )
          { 
           cout<<"bad evt   gtid "<<rev.GetGTID()<<" "<<iEntry<<" "<<posX<<", "<<posY<<", "<<posZ<<endl;
          }
          if(posZ>-3040 && posZ<-3000 && sqrt(posX*posX+posY*posY)<150 && sqrt(posX*posX+posY*posY)>100)
          { 
           cout<<"good evt  gtid "<<rev.GetGTID()<<" "<<iEntry<<" "<<posX<<", "<<posY<<", "<<posZ<<endl;
          }
