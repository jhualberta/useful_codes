...
void analyMPpmts()
{  
   const char *filename = "FitMPScint_pdfcompress_direction_6MeVbeta_center_+x.root";
   ...
   vector<TH1F*> hlistTimeWin1;
   vector<TH2F*> hlistPMTdist1;
   vector<TH2F*> hlistTResVsCosTheta1;
   for(int i = 0;i<20;i++)
   {
     TH1F *htemp = new TH1F("htemp","angular distribution of pmt, expand window",200,-1,1);
     hlistTimeWin1.push_back((TH1F*)htemp->Clone(Form("hcosTheta_cutwin1_%u",i)));
     delete htemp;
     TH2F *htemp2 = new TH2F("htemp2","phi theta distr of pmt, expand window",628, -TMath::Pi(), TMath::Pi(), 200,-1,1);
     hlistPMTdist1.push_back((TH2F*)htemp2->Clone(Form("hPhiTheta_cutwin1_%u",i)));
     delete htemp2;
     TH2F* htemp3 = new TH2F("htemp3","tRes vs cosTheta",400,-100,300,200,-1,1);
     hlistTResVsCosTheta1.push_back((TH2F*)htemp3->Clone(Form("htResCosTheta_cutwin1_%u",i)));
     delete htemp3;
   }

   vector<TH1F*> hlistTimeWin2;
   vector<TH2F*> hlistPMTdist2;
   vector<TH2F*> hlistTResVsCosTheta2;
   for(int i = 0;i<7;i++)
   {
     TH1F *htemp = new TH1F("htemp","angular distribution of pmt, shrink window",200,-1,1);
     hlistTimeWin2.push_back((TH1F*)htemp->Clone(Form("hcosTheta_cutwin2_%u",i)));
     delete htemp;
     TH2F *htemp2 = new TH2F("htemp2","phi theta distr of pmt, shrink window",628, -TMath::Pi(), TMath::Pi(), 200,-1,1);
     hlistPMTdist2.push_back((TH2F*)htemp2->Clone(Form("hPhiTheta_cutwin2_%u",i)));
     delete htemp2;
     TH2F* htemp3 = new TH2F("htemp3","tRes vs cosTheta",400,-100,300,200,-1,1);
     hlistTResVsCosTheta2.push_back((TH2F*)htemp3->Clone(Form("htResCosTheta_cutwin2_%u",i)));
     delete htemp3;
   }

   vector<TH1F*> hlistTimeWin3;
   vector<TH2F*> hlistPMTdist3;
   vector<TH2F*> hlistTResVsCosTheta3;
   for(int i = 0;i<11;i++)
   {
     TH1F *htemp = new TH1F("htemp","angular distribution of pmt, shift window",200,-1,1);
     hlistTimeWin3.push_back((TH1F*)htemp->Clone(Form("hcosTheta_cutwin3_%u",i)));
     delete htemp;
     TH2F *htemp2 = new TH2F("htemp2","phi theta distr of pmt, shift window",628, -TMath::Pi(), TMath::Pi(), 200,-1,1);
     hlistPMTdist3.push_back((TH2F*)htemp2->Clone(Form("hPhiTheta_cutwin3_%u",i)));
     delete htemp2;
     TH2F* htemp3 = new TH2F("htemp3","tRes vs cosTheta",400,-100,300,200,-1,1);
     hlistTResVsCosTheta3.push_back((TH2F*)htemp3->Clone(Form("htResCosTheta_cutwin3_%u",i)));
     delete htemp3;
   }
             ...
             for(unsigned int ipmt=0;ipmt<calpmts.GetCount();ipmt++)
             {
               ...
               //if( iEntry == 1 )
               {
                 ...
                 if( hitTime<modeTime+100 && hitTime>modeTime-50 )
                 {
                   ...
                   for(int i =0;i<20;i++) {
                    // enlarge window:[-5,-1],[-5,-0.5],...,[-5,5]
                    if(-6<tRes && tRes<-1+i*0.5) {// tRes windows: [-5,-1] to [-5,5]
                      hlistTimeWin1[i]->Fill(Xdiff*u_mc);
                      hlistPMTdist1[i]->Fill(pmtpos.Phi(), pmtpos.CosTheta());
                      hlistTResVsCosTheta1[i]->Fill(tRes,Xdiff*u_mc);
                    }
                   }
                   for(int i=0;i<7;i++) {
                    // shrink window: [-5,-1],[-5,-1.5],...,[-5,-4.5]
                    if(-6<tRes && tRes<-1-i*0.5) { 
                      hlistTimeWin2[i]->Fill(Xdiff*u_mc);
                      hlistPMTdist2[i]->Fill(pmtpos.Phi(), pmtpos.CosTheta());
                      hlistTResVsCosTheta2[i]->Fill(tRes,Xdiff*u_mc);
                    }
                   }

                   for(int i=0;i<11;i++) {
                    // shrink window: [-5,-1],[-5,-1.5],...,[-5,-4.5]
                    if(-6+i*0.5<tRes && tRes<-1+i*0.5) {
                      hlistTimeWin3[i]->Fill(Xdiff*u_mc);
                      hlistPMTdist3[i]->Fill(pmtpos.Phi(), pmtpos.CosTheta());
                      hlistTResVsCosTheta3[i]->Fill(tRes,Xdiff*u_mc);
                    }
                   }
...
//  hcosTheta->Draw();
//  hcosTheta_cut->Draw("sames");
  for (size_t j=0;j<hlistTimeWin1.size();j++)
  { 
    hlistTimeWin1[j]->Write();
    hlistPMTdist1[j]->Write(); 
    hlistTResVsCosTheta1[j]->Write();
  }

  for (size_t j=0;j<hlistTimeWin2.size();j++)
  {
    hlistTimeWin2[j]->Write();
    hlistPMTdist2[j]->Write();
    hlistTResVsCosTheta2[j]->Write();
  }

  for (size_t j=0;j<hlistTimeWin3.size();j++)
  {
    hlistTimeWin3[j]->Write();
    hlistPMTdist3[j]->Write();
    hlistTResVsCosTheta3[j]->Write(); 
  }
