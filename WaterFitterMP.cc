  ...
  : Processor("waterFitterMP")
{
  ...
  fDirectionModeCut = PMTSelectorFactory::Get()->GetPMTSelector( "modeCut" );
  ...
  DBLinkPtr fDriveCorrection = DB::Get()->GetLink("FIT_MULTIPATH");
                    ...
                    DS::EV& ev )
{
  ...
  try
    {
      if( ev.GetCalPMTs().GetCount() > fCutOff )
        ...
        {
          detail << "WaterFitter::Event: insufficient points for quad to run, exiting" << newline;
    ...
    debug << "WaterFitter::Event: Seed failed " << error.what() << ", exiting." << newline;
  ...
  /// Run the position time direction method
  ...
  try
    {
      ...
      FitVertex directionVertex = directionResult.GetVertex(0);
