...
using namespace std;
void AnalyScintMPTres_allMP6189()
{
  const char* filename = "FitRat_scint_10MeVbeta_innerAViso_2e3evt.root";
 ...
 //!!! RSP correctiion
  ...
  tree->Branch("gCosPMTMC", &gCosPMTMC);
  ...
  for(size_t iEntry = 0; iEntry < numEvents; iEntry++ )
  {
    ...
    if (iEVCount) {
     for(size_t iEV=0; iEV<iEVCount; iEV++) //iEVCount
     {
	     ...
	     TVector3 u_fit;
	     const string fitName = "scintFitter";
             pvfecd->clear();
             ...
             if(nhits>15) // nhit cut for cosTheta calculation!!
             {
                  ...
                  {  // It needs to exist
                    try {//!!! fVertex somehow broken here 
                     RAT::DS::FitVertex fVertex = ev.GetFitResult("scintFitter").GetVertex(0);
		     if( fVertex.ValidPosition() )
		     {
                      //std::cout<<"eventGTID "<<eventGTID<<" "<<iEntry<<std::endl;
                      pos_fit = fVertex.GetPosition(); 
          
                      RAT::DS::FitVertex fVertex1 =ev.GetFitResult("scintFitter").GetVertex(0);
                      // if(!fVertex1.ValidDirection()) continue;
		      ...
		      if( !ev.ClassifierResultExists("ITR:scintFitter") ) continue;
                      if ( !ev.GetClassifierResult( "ITR:scintFitter" ).GetValid() ) continue;
                      itr = ev.GetClassifierResult( "ITR:scintFitter" ).GetClassification( "ITR" );

                      if( !ev.ClassifierResultExists("isotropy:scintFitter") ) continue;
                      if ( !ev.GetClassifierResult( "isotropy:scintFitter" ).GetValid() ) continue;
                      iso = ev.GetClassifierResult( "isotropy:scintFitter" ).GetClassification( "snobeta14" );

                      if( !ev.ClassifierResultExists("isotropy:scintFitter") ) continue;
                      if ( !ev.GetClassifierResult( "isotropy:scintFitter" ).GetValid() ) continue;
                      thij = ev.GetClassifierResult( "isotropy:scintFitter" ).GetClassification( "thetaij" );
		      ...
		      for(unsigned int ipmt=0;ipmt<calpmts.GetCount();ipmt++)
                      {
                         ...
                         gCosPMTMC->push_back(cosThetaMC);
		      } //loop PMT
		      triggerWord = TriggerType; // !!! Fill trigType
                      tree->Fill();
                }//fit Valid Position
	      } //try catch position
             catch(exception& e)
