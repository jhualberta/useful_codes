...
PI = np.pi
radius = 850
QmaxCut = 100 ## the qmax PMT must > 100 pC
QminCut = 20 ## the minimum q of selected PMT must > 20 pC

print "now processing %s\n"%(sys.argv[1])
file0 = str(sys.argv[1])
fout = TFile("OptTest_"+file0,"RECREATE");
...
for Files in AllFiles:
	...
	# Low Level cuts
			...
			pmtTime = list(pmtTime)
                        ...
                        check_charges = [] ## to remove zeros, but still need to loop up charges
                        check_pmtTime = []
			...
			for i in range(255):
                             if charges[i] > 0: ### Fill all the PMTs in this event
                                  ...
                                  check_pmtPhi.append(savePmtPhi[i]);
                                  check_pmtCosTheta.append(savePmtCosTheta[i]); 
			...
			Time_CosTheta_Phi3.Reset()
                        ...
                        idx_selectQ_pmtList = [] ## save the indices of the selected 6 PMTs
                        ### in0 is the largest, in1 is the 2nd largest, ... (reversed sorting)
                        for i in range(len(selectQ_pmtList)):
                             ind = charges.index(selectQ_pmtList[i])
                             idx_selectQ_pmtList.append(ind)
                         
                        # print "is index correct??", idx_selectQ_pmtList 
                        select_pmtPos = [] ## from 0 to 6
                        for i in range(len(idx_selectQ_pmtList)):
                             phi = Phi[idx_selectQ_pmtList[i]] 
                             cost = CosTheta[idx_selectQ_pmtList[i]]
                             th = arccos(cost)
                        ...
                        searchPMTid_cluster1.append(charges.index(selectQ_pmtList[0])) # save the qmax first; this is the seed of the 1st cluster
                        seed_cluster2_idx = -10000 # if there is a PMT far away from qmax or q0 PMT, is this PMT the seed of the 2nd cluster?
