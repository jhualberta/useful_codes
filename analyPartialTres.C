...
void analyPartialTres()
{  
   const char* filename = "Analysis50_r0000250127_s000_p000.root";
	      ...
	      if(calpmts.GetFECDPMT(ipmt).GetID()==fecdID)
	      {
		   ...
		   //hMfit->Fill(dPSUPfit); //distance
			...
			{ htResInScint->Fill(tRes);
		          if(nhits>100) htResInScint_nhit100->Fill(tRes);
		          if(nhits>300) htResInScint_nhit300->Fill(tRes);
                        }
                        else{ 
                        htResInWater->Fill(tRes);
                        if(nhits>100) htResInWater_nhit100->Fill(tRes);
