...
void analyPartialMC()
{  
   const char* filename = "FitMP_simu_3MeVbeta_2p0PPOoxford_IsoTop_splitZ3000.root";
...
//check pos.Mag() vs fitTime
  ...
  //for N16 source 
   ...
   TH1F* hRadialBias = new TH1F("hRadialBias","SNO+  (Xfit-Xmc)*Xmc.Unit()", 2000,-9000,9000);
   ...
   TH1F* hNhits_trig = new TH1F("hNhits_trig","Nhits trig",1000,0,3000); 
   ...
   for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
   {
    ...
    for(Int_t iev=0;iev<nevC; iev++) {
     ...
     if(trig!=0)
     {
      ...
      if(rev.FitResultExists("multipath")){//find multipath exists
       try{
         ...
         {
         //rev.GetFitResult("multipath").GetFOMNames()[0];
         //std::string FOMname ="multipath";
         //double fomValue =  rev.GetFitResult("multipath").GetFOM(FOMname);
	  ...
	  posfit_cor.SetXYZ(pos_fit.X(),pos_fit.Y(),pos_fit.Z()-108); 
          ...
          //ITR:multipath QPDT:multipath isotropy:multipath

          //hFOM->Fill(fomValue); hFOMvsNhit->Fill(rev.GetNhits(),fomValue);//cout<<rev.GetNhits()<<" "<<fomValue<<endl;

         // RAT::DS::FitVertex fVertex1 = rev.GetFitResult("multipathdirection").GetVertex(0);
         ...
         // } // if valid direction
	      ...
	      if(calpmts.GetFECDPMT(ipmt).GetID()==fecdID)
	      {
		   ...
		   hfitRhoZ->Fill(sqrt(posX*posX+posY*posY),posZ);
                   hNhits_trig->Fill(nhits);
                   hRadialBias->Fill( (pos_fit-pos_mc)*pos_mc.Unit() );

                   if(pos_mc.Mag()<6005 && pos_mc.Z()>trueLevel && pos_mc.Z()<3000)
                   {
		     countMC++;
		   ...
		   //hMfit->Fill(dPSUPfit); //distance
                   bool condition = (pos_fit.Mag()<6000 && pos_fit.Z()>3000);
  ...
  gx->SetLineColor(kRed);gy->SetLineColor(kRed);gz->SetLineColor(kRed);gR->SetLineColor(kRed);
  hDeltaX->Fit(gx,"R");
  hDeltaY->Fit(gy,"R");
  hDeltaZ->Fit(gz,"R");
  hRadialBias->Fit(gR,"R"); 
...
//  std::cout<<"fitValid && FECD/FECD total(9188+9027): "<<countSuccess<<"/"<<hFECD->GetEntries()<<" ratio: "<<countSuccess/hFECD->GetEntries()*100<<std::endl; //fitValid && FECD/ FECD, FECD means FECD == 9188, FECD total means 9188+9207
  ...
  std::cout<<"mc "<<countMC<<" fit "<<countFit<<std::endl;
