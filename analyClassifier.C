...
void analyClassifier()
{  
   const char* filename = "FitPartialRat_fillbottom.root";
   ...
   TH1F* hThetaCut1 = new TH1F("hcosThetaCut1SNO_Rat", "SNO+ Rat fitter (fitPos-sourcePos)*fitDirec, |fitPos-sourcePos|>1.5 m", 1000,-1,1);
   TH1F* hThetaCut2 = new TH1F("hcosThetaCut2SNO_Rat", "SNO+ Rat fitter (fitPos-sourcePos)*fitDirec, |fitPos-sourcePos|>1.0 m", 1000,-1,1);
   TH1F* hThetaCut3 = new TH1F("hcosThetaCut3SNO_Rat", "SNO+ Rat fitter (fitPos-sourcePos)*fitDirec, |fitPos-sourcePos|>0.75m", 1000,-1,1);
   TH1F* hThetaCut4 = new TH1F("hcosThetaCut4SNO_Rat", "SNO+ Rat fitter (fitPos-sourcePos)*fitDirec, |fitPos-sourcePos|>0.5 m", 1000,-1,1);
   TH1F* hThetaCut5 = new TH1F("hcosThetaCut5SNO_Rat", "SNO+ Rat fitter (fitPos-sourcePos)*fitDirec, |fitPos-sourcePos|>1.2 m", 1000,-1,1);
...
//check pos.Mag() vs fitTime
  ...
  //for N16 source 
   ...
   TH1F* hfitX = new TH1F("hfitX", "SNO+ Rat fitter X, FECD cut", 2000,-9000,9000);   
   TH1F* hfitY = new TH1F("hfitY", "SNO+ Rat fitter Y, FECD cut", 2000,-9000,9000);  
   TH1F* hfitZ = new TH1F("hfitZ", "SNO+ Rat fitter Z, FECD cut", 2000,-9000,9000);
   TH2F* hfitXY = new TH2F("hfitXY", "SNO+ Rat fitter X vs Y, FECD cut", 2000,-9000,9000,2000,-9000,9000); 
   TH2F* hfitXZ = new TH2F("hfitXZ", "SNO+ Rat fitter X vs Z, FECD cut", 2000,-9000,9000,2000,-9000,9000);
   TH2F* hfitRZ = new TH2F("hfitRZ", "SNO+ Rat fitter R vs Z, FECD cut", 1000, 0,9000,2000,-9000,9000);
   TH1F* hPosMag = new TH1F("hPosMag","SNO+ Rat fitter Mag()",1000,0,10000);

   TH1F* hfitX_trig = new TH1F("hfitX_trig", "SNO+ Rat fitter X, FECD+trig cut", 2000,-9000,9000);
   TH1F* hfitY_trig = new TH1F("hfitY_trig", "SNO+ Rat fitter Y, FECD+trig cut", 2000,-9000,9000);
   TH1F* hfitZ_trig = new TH1F("hfitZ_trig", "SNO+ Rat fitter Z, FECD+trig cut", 2000,-9000,9000);
   TH2F* hfitXY_trig = new TH2F("hfitXY_trig", "SNO+ Rat fitter X vs Y, FECD+trig cut", 2000,-9000,9000,2000,-9000,9000);
   TH2F* hfitXZ_trig = new TH2F("hfitXZ_trig", "SNO+ Rat fitter X vs Z, FECD+trig cut", 2000,-9000,9000,2000,-9000,9000);
   TH2F* hfitRZ_trig = new TH2F("hfitRZ_trig", "SNO+ Rat fitter R vs Z, FECD+trig cut", 1000, 0,9000,2000,-9000,9000);
   TH1F* hPosMag_trig = new TH1F("hPosMag_trig","SNO+ Rat fitter Mag(), FECD+trig cut",1000,0,10000);
   ...
   TH1F* hbeta14 = new TH1F("hbeta14", "beta14", 200,-10,10);
   TH1F* hthetaij = new TH1F("hthetaij", "theta_ij",200,-10,10);
   TH1F* hEnergy = new TH1F("hEnergy", "fitted energy",100,0,100);
   ...
   //Double_t grVelocity =  183.503698029;//2.17554021555098529e+02 ;//light water:2.17554021555098529e+02; heavy water:2.18254558686904687e+02
   Double_t rPSUP = 8390;
   ...
   for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
   {
	...
	if(calpmts.GetFECDPMT(ipmt).GetID()==fecdID)
      ...
      if(rev.FitResultExists("partialFitter"))
      {//find partialFitter exists
  	      ...
  	      if(calpmts.GetFECDPMT(ipmt).GetID()==fecdID)
  	      {
  		   ...
  		   hPhiTheta->Fill(pos_fit.Phi(),pos_fit.CosTheta());
                   ...
                   if( fitVertex.ValidEnergy() )
                     hEnergy->Fill(fitVertex.GetEnergy());

                   if( rev.ClassifierResultExists("ITR:partialFitter") ) {
                     if ( rev.GetClassifierResult( "ITR:partialFitter" ).GetValid() ) {
                       itr = rev.GetClassifierResult( "ITR:partialFitter" ).GetClassification( "ITR" );
                       hitr->Fill(itr); 
                      }
                   }

                   if( rev.ClassifierResultExists("isotropy:partialFitter") ) { 
                     if ( rev.GetClassifierResult( "isotropy:partialFitter" ).GetValid() ) { 
                        iso = rev.GetClassifierResult( "isotropy:partialFitter" ).GetClassification( "snobeta14" );
                        thij = rev.GetClassifierResult( "isotropy:partialFitter" ).GetClassification( "thetaij" );
                        hbeta14->Fill(iso); hthetaij->Fill(thij);
                     }
                   }
...
//  std::cout<<"fitValid && FECD/FECD total(9188+9027): "<<countSuccess<<"/"<<hFECD->GetEntries()<<" ratio: "<<countSuccess/hFECD->GetEntries()*100<<std::endl; //fitValid && FECD/ FECD, FECD means FECD == 9188, FECD total means 9188+9207
  ...
  hitr->Write();hbeta14->Write();hthetaij->Write();
  hEnergy->Write();
  ...
  hFOM->Write();hFitTime->Write();hHitTime->Write();htRes->Write();htRes_trig->Write();htResTrue_trig->Write();
