...
#include <string>
#include <vector>
void AnalysisMPW655(const char* filename)
{
...
//  TNtuple* candidateEventInfo = new TNtuple("candidateEventInfo", "Information for interesting events, without drive correction", "runNumber:eventGTID:nHit:radius:cosThetaToSun:x_pos:y_pos:z_pos:x_dir:y_dir:z_dir:itr:iso");
  ...
  double posx, posy, posz, posRad, energy, time, dirx, diry, dirz, sunDirX, sunDirY, sunDirZ, cosThetaToSun, ITR, beta14, itr, iso, thij;
		    ...
		    {  // It needs to exist
		     ...
		     if( fVertex.ValidPosition() )
		     {
		       ...
		       //radiusCor->Fill(radiusOfEventCor);
                       ...
                       if( !ev.GetFitResult("waterFitterMP").GetVertex(0).ContainsEnergy() ) continue;
		       RAT::DS::FitResult fResult = ev.GetFitResult("waterFitterMP");
