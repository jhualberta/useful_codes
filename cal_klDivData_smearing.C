...
//// calculate KL-div
//  hpdf1->Draw();
  const char* filename = "Merged_ExtractTres_WaterMP6176_nhit15_GeneralPhysicsMC_WaterSolar_NumuRun_r200004to207718.root";
...
// Merged_MP_ExtractTres_WaterMP6176_nhit15_GeneralPhysicsMC_WaterTl208Run_r200004to203602_s0_p0.root
  ...
  ffname0 = "GetSolarMC_smearDirUp_5to15MeV_"+ffname0;
  ...
  for(int i =0;i<Tdata->GetEntries();i++)
  {
    ...
    /*!!!!! smearing direction: +0.101/-0.013 --------------------------
    //    /// costheta' = -1+(costheta-1)*(1+delta),  /// -costhetaToSun' = 1+(-costhetaToSun-1)*(1+delta) => costhetaToSun' = -1+(costhetaToSun+1)*(1+delta)
    */


    // double deltaTheta = 0.101; // change sign up
    // double deltaTheta = -0.013; // change sign down
