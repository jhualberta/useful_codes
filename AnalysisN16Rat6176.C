...
#include <string>
#include <vector>
void AnalysisN16Rat6176(const char* filename)
{
		  ...
		  // Fill the nHit histogram, all events should have nHit
	       ...
	       Double_t radiusOfEvent = -1., radiusOfEventCor = -1.;
               ...
               for(unsigned int ipmt=0;ipmt<calpmts.GetFECDCount();ipmt++)//do FECD cuts
               {
                  ...
                  if (fecdID == 9188)
                  //cout<<"fecd "<<fecdID<<endl;
		  {
		    bool checkFitExist = ev.FitResultExists(fitName);
		    //cout<<"check "<<checkFitExist<<endl;
		    ...
		    {  // It needs to exist
                     ...
                     if( !ev.GetFitResult("waterFitter").GetVertex(0).ContainsPosition() ) continue;
                     if(!fVertex.ValidPosition()) continue;
                     if( !ev.GetFitResult("waterFitter").GetVertex(0).ContainsDirection() ) continue;
		     ...
		     energy = eCorr.CorrectEnergyRSP(energyOrigin,2);
                     ...
                     //TString ss1 = fResult.GetFOMNames()[0];
